...
from tg_bot_localhost_async.keyboards import get_keyboard, get_callback_btns
from tg_bot_localhost_async.middlewares import AuthSession
from tg_bot_localhost_async.orm_queries import (
...
async def all_advs_auth_page1(message: types.Message, session: AsyncSession, auth_token: str):
    ...
    cb_btns = page1[1]
    cb_btns = {
        k: v + "_advs-all_auth" for k, v in cb_btns.items()
    }  # записываем тип множества в cb кнопки + признак auth
    await message.answer(
